...
export default function AllKuppiQuestions() {
  ...
  const fetchKuppiQuestions = async () => {
    try {
      ...
      setKuppiQuestions(response.data.filter(kuppi => kuppi.type === 'question'));
    } catch (error) {
  ...
  return (
      <div className="container mt-5 mb-5 bg-light p-5 rounded rounded-5">
        <center><Button variant="outline-success" className="w-50 p-3" onClick={() => setShowPostModal(true)}>Add Question</Button></center>

        <Modal show={showPostModal} onHide={handlePostModalClose}>
          <Modal.Header closeButton>
            <Modal.Title>Add Kuppi Question</Modal.Title>
          </Modal.Header>
          <Modal.Body>
        ...
        <div className="mt-4">
          <h2 className="text-center mb-4"><span className="text-danger">Kuppi </span><span
              className="text-success"> Questions</span></h2>
                ...
                <Card key={kuppi._id} className="m-3" style={{ width: '18rem' }}>
                  ...
                  {kuppi.author === loggedInUserId && (
                      <Card.Footer>
                        ...
                        <Button variant="danger" className="ms-2" onClick={() => handleDelete(kuppi._id)}>Delete</Button>
                      </Card.Footer>
                  )}
                </Card>
            ))}
